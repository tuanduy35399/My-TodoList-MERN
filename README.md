# ✅ Todo List App (MERN STACK)

Xin chào 👋 Đây là ứng dụng Todo List cơ bản được xây dựng bằng MERN STACK
Ứng dụng giúp mình quản lý công việc hằng ngày: thêm, sửa, xóa và lưu trữ công việc vào cơ sở dữ liệu MongoDB, đây là dự án đầu tay trong quá trình mình học fullStack. 

---

## 🚀 Tính năng

- ➕ Thêm công việc mới
- 📝 Chỉnh sửa trực tiếp công việc ngay trên danh sách
- ❌ Xóa công việc
- 💾 Lưu trữ công việc với LocalStorage (không mất khi reload trang)
- 📱 Responsive Design (hiển thị tốt trên desktop và mobile)
- 🎨 Giao diện đơn giản, hiện đại với hiệu ứng hover
---

## 🖼️ Demo giao diện

![Ảnh demo](./public/demoweb.png)

---

## 🛠️ Công nghệ sử dụng
- Phía Frontend:
+ ⚛️ [ReactJS](https://react.dev/) – Framework chính
+ 🎨 HTML/CSS/JavaScript
+ 🎨 Thư viện Sonner
- Phía Backend:
+ Node.js, Express.js
+ dotenv, MongoDB/ mongoose
- 📧 EmailJS gochoctapcuatuanduy@gmail.com

---
## Demo (bản localStorage không có backend): https://tuanduy35399.github.io/My-TodoList-LocalStorage/
---

## 📦 Cài đặt & Chạy dự án

### Clone project về máy:

```bash
git clone https://github.com/tuanduy35399/My-TodoList-LocalStorage.git
```

### Truy cập vào thư mục chứa dự án
```bash
cd My-TodoList-MERN
```

### Tải các module cần thiết
```bash
npm i
```

### Khởi chạy dự án
Ở folder frontend:
```bash
npm run dev
```

Ở folder backend:
```bash
npm start //để chạy node
hoặc
npm run dev //để chạy nodemon
```
